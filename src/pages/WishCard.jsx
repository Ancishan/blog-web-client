            ...
            <div className="card w-full bg-base-100 shadow-xl">
              ...
              <div className="card-body">
                <div className="flex justify-between items-center align-middle text-center">
                  <h1 className=" text-2xl font-semibold text-blue-500">{blog?.blog_title}</h1>
                  <div className="badge badge-secondary bg-blue-500 py-3">{blog?.category}</div>
                </div>
                <p>{blog?.short_description}</p>
                <p>{blog?.description}</p>
                <div className="card-actions">
                  <p className='text-blue-400 font-bold'>{blog?.date && new Date(blog.date).toLocaleDateString()}</p>
                  <button type="button" onClick={handleDelete} className="text-red-600 font-bold">Delete</button>
                </div>
              </div>
            </div>
          )}
